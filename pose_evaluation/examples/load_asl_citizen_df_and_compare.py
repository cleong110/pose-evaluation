# TODO: distance cache and metrics https://colab.research.google.com/drive/1Ztu20aVnpb_j27t3IGV9IhcR8wB5Ovyw?usp=sharing
# https://chatgpt.com/share/67dda2d1-bf3c-800e-a337-cfa9f8566fd1
import json
import time
...
if __name__ == "__main__":
    ...
    print(df.head())
    print(df.info())
    unique_glosses = pd.unique(df["Gloss"])
    print(f"{len(unique_glosses)} glosses:{unique_glosses}")
    pose_files = pd.unique(df["File Path"])
    print(f"{len(unique_glosses)} glosses:{unique_glosses}")
    ...
    for gloss in unique_glosses:
        ...
        hyp_poses = [
            get_pose_data(pose_path) for pose_path in tqdm(hyp_pose_paths, desc=f"loading files for {gloss_a}")
        ]

        ref_pose_paths = sign_b_df["File Path"].tolist()
        ref_poses = [
            get_pose_data(pose_path) for pose_path in tqdm(ref_pose_paths, desc=f"loading files for {gloss_b}")
        ]

        other_class_count = len(sign_a_df) * 4  # as in
        random_sample_df = not_a_or_b_df.sample(n=len(sign_a_df), random_state=42)
        ...
        for metric in metrics:
            ...
            results_path = data_folder / "scores" / f"{gloss_a}_{gloss_b}_{metric.name}_score_results.csv"
            ...
            for hyp, hyp_pose_path in tqdm(
                zip(hyp_poses, hyp_pose_paths), desc=f"{metric.name} scoring {gloss_a} vs random glosses"
            ):
